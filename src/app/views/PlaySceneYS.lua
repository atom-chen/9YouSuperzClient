...
local PlaySceneYS = class("PlaySceneYS",function()
...
function PlaySceneYS:ctor(enterRoomMsgTbl)
	--清理纹理
	...
	-- 加载界面资源
	local csbName = "csd/PlaySceneYS.csb"
	if self.roomChairs == gt.GameChairs.TEN then
		csbName = "csd/PlaySceneTenYS.csb"
	elseif self.roomChairs == gt.GameChairs.EIGHT then
		csbName = "csd/PlaySceneEightYS.csb"
	elseif self:isTwelve() then
        csbName = "csd/PlaySceneTwelveYS.csb"
	end
	...
	-- self.rootNode:reorderChild(self.decisionBtnNode,PlaySceneYS.ZOrder.DECISION_BTN)
	...
	self.rootNode:reorderChild(chatBgNode,PlaySceneYS.ZOrder.CHAT)
	...
	gt.addBtnPressedListener(self.chatBtn,function()
		...
		self:addChild(chatPanel,PlaySceneYS.ZOrder.CHAT)
	end,"common/ys/btnclick")
	...
	gt.addBtnPressedListener(self.settingBtn,function()
		...
		local settingPanel = require("app/views/Setting"):create(param,3, self:isTwelve())
		self:addChild(settingPanel,PlaySceneYS.ZOrder.SETTING)
	end,"common/ys/btnclick")
	...
	self.yuyinChatNode:setLocalZOrder(PlaySceneYS.ZOrder.CHAT)
...
function PlaySceneYS:isScoreSpecial()
	return true
end

function PlaySceneYS:isScoreSpecial11()
...
function PlaySceneYS:showBuyPledge()
...
function PlaySceneYS:hideBuyPledge()
	if self:isTwelve() then
		return
	end
...
function PlaySceneYS:hidePopFrame()
	self.node_buyPledge:setVisible(false)
	self.bClickPop = false
end

function PlaySceneYS:initBuyPledge()
	if self:isTwelve() then
		return
	end
...
function PlaySceneYS:initBuyPledgeBySetting()
	if self:isTwelve() then
		return
	end
...
function PlaySceneYS:initDecision()
	...
	if not self.isInitDecision then
		...
		-- 隐藏玩家押分按钮
		if self:isScoreSpecial11() then
			self.decisionBtnNode =  gt.seekNodeByName(self.rootNode,"Node_decisionBtn"):getChildByName("Node_ScoreSpecial")
		else
			self.decisionBtnNode = gt.seekNodeByName(self.rootNode,"Node_decisionBtn"):getChildByName("Node_Score")
		end
		self.rootNode:reorderChild(self.decisionBtnNode,PlaySceneYS.ZOrder.DECISION_BTN)
		self.decisionBtnNode:setVisible(false)
...
function PlaySceneYS:isTwelve()
	return self.roomChairs == gt.GameChairs.TWELVE
end

function PlaySceneYS:unregisterAllMsgListener()
...
function PlaySceneYS:changePlayBg()
	...
	-- 默认值
	if cc.UserDefault:getInstance():getStringForKey("YSDesktop") == "" or cc.UserDefault:getInstance():getStringForKey("YSDesktop") == nil then
	...
	if desktop == "3" or desktop == "4" then
		self.card_back = "image/card2/back3.png"
	else
		self.card_back = "image/card2/back3.png"
	end
	if self.cardViews ~= nil then
		for k,v in pairs(self.cardViews) do
			for kk,vv in pairs(v.imgCards) do
				...
				if filename == "image/card2/back3.png" or filename == "image/card2/back3.png" then
...
function PlaySceneYS:showEmojiAnimation(emojiAnimation,seatIdxStart,id,user_id)
...
function PlaySceneYS:onNodeEvent(eventName)
...
function PlaySceneYS:update(delta)
	...
	if #(self.cardShowQueue) > 0 then
		if self.cardShowDelay <= 0 then
			...
			if roomPlayer == nil then return end
			...
			if self.tableSetting and self.tableSetting.options and tonumber(gt.charAt(self.tableSetting.options,15)) > 0 then
...
--更新自动托管等待时间
function PlaySceneYS:updateAutoWaitCD(delta)
	delta = delta or 0
	if self.IsPlay and self.PlaySpeed then
		delta = self.IsPlay*self.PlaySpeed*delta
	end
...
--是否庄家
function PlaySceneYS:isBanker(playerSeatIdx)
...
function PlaySceneYS:isRoomOwner()
	--房间创建者，房间庄家都是
...
--进入房间回调
function PlaySceneYS:onRcvEnterRoom(msgTbl)
	...
	--如果是房主 显示开始按钮
	if self.isRoomCreater and (self.roomState == ROOM_STATE.INIT or self.roomState == ROOM_STATE.READY) then
		self.startBtn:setVisible(true)--默认posX-150
		self.startBtn:setEnabled(self.roomState == ROOM_STATE.READY)

		if self.isMatch > 0 and self.IsNewGame then
			self.readyBtn:setPositionX(150)
			self.readyBtn:getChildByName("Text_Tip"):setString("继续游戏")
			self.startBtn:setPositionX(-150)
		else
			self.readyBtn:setPositionX(150)
		end
	else
...
--接收房间添加玩家消息
function PlaySceneYS:onEnterRoomOther(msgTbl)
	...
	local function GetDisplayIndex(seat,reseat)
		...
		elseif self.roomChairs == gt.GameChairs.TWELVE then
			displayseat = { 7, 8, 6, 9, 5, 10, 4, 11, 3, 12, 2, 1 }
		end
...
-- 退出房间
function PlaySceneYS:onRcvExitRoom(msgTbl)
	if msgTbl.player == gt.playerData.uid then
		-- 自己退出
		gt.removeLoadingTips()
		...
		if (msgTbl.seat >= 0) then
			...
			self.buyPledgeSigns[roomPlayer.displaySeatIdx]:setVisible(false)
...
-- 断线重连
function PlaySceneYS:onRcvSyncRoomState(msgTbl,isStart)
	gt.removeLoadingTips()
	self:stopAllActions()
	--关闭搓牌动画界面
		...
		if v.player == gt.playerData.uid then
			...
			or msgTbl.room_state == ROOM_STATE.SETTLE_ROUND) then
				...
				if not v.is_wait and v.state ~= PLAYER_STATE.READY then
					...
					self.readyBtn:setVisible(true)
					if msgTbl.room_state ~= ROOM_STATE.INIT then
						self.readyBtn:setPositionX(0)
					end
					if not gt.isIOSReview() then
						self.inviteFriendBtn:setVisible(true)
			...
			and v.state == PLAYER_STATE.READY and not v.is_wait then
				...
				if self.roomState == ROOM_STATE.READY then
					self.startBtn:setVisible(self.isRoomCreater)
					self.startBtn:setEnabled(self.isRoomCreater)
					self.readyBtn:setPositionX(0)
				end
				if not gt.isIOSReview() then
					self.inviteFriendBtn:setVisible(true)
...
function PlaySceneYS:showHandCardsByQueue(roomPlayer)
	--local data = {displaySeatIdx=displaySeatIdx, cards=cards, sex=sex, niu_type=niu_type}
	table.insert(self.cardShowQueue,roomPlayer)
end

--显示手牌
function PlaySceneYS:showHandCards(bOrbitAni,displaySeatIdx,cards,cardsigns,sex,niu_type,bShowAll)
				...
				if (i > self.tableSetting.cards_count) then
					if (bOrbitAni) then
						...
						local change = cc.CallFunc:create(function(sender)
							...
							sender:loadTexture(string.format("image/card2/%02x.png",value),ccui.TextureResType.plistType)
						end)
						...
						cardView.imgCards[i]:loadTexture(string.format("image/card2/%02x.png",v),ccui.TextureResType.plistType)
					end
				else
					--亮牌时牌的顺序会变
					cardView.imgCards[i]:loadTexture(string.format("image/card2/%02x.png",v),ccui.TextureResType.plistType)
	...
	if #cards > 0 and niu_type ~= nil then
		...
		-- 	end						
		-- end
		cardView.imgNiuType:loadTexture(string.format("image/playys/niu/%s.png",gt.NIU_NAME[niu_type + 1]),ccui.TextureResType.plistType)
		local pathNiuRate = gt.getNiuRateResPath(self.tableSetting.double_type,niu_type,false,"image/playys/niu/")
		if pathNiuRate then
			cardView.imgNiuType:setPositionX(63)
		else
			cardView.imgNiuType:setPositionX(105)
		end
		cardView.imgNiuRate:setVisible(pathNiuRate and true or false)
...
--玩家准备回调
function PlaySceneYS:onRcvReady(msgTbl)
	--self.roomState = ROOM_STATE.READY
	local seatIdx = msgTbl.seat + 1
	self:playerGetReady(seatIdx)
	if seatIdx == 1 and not self.isWait and not self:isTwelve() then
		self.startBtn:stopAllActions()
		self.startBtn:runAction(cc.MoveTo:create(0.5,cc.p(0,-110)))
	end
	...
	if (self.isWait and seatNum >= self.roomChairs and not self:isTwelve()) then
		self.readyBtn:setVisible(false)
		self.startBtn:stopAllActions()
		self.startBtn:runAction(cc.MoveTo:create(0.5,cc.p(0,-110)))
	end
...
--玩家暂停回调
function PlaySceneYS:onRcvPause(msgTbl)
	local seatIdx = msgTbl.seat + 1
...
function PlaySceneYS:onRcvReadyWait(msgTbl)
	local seatIdx = msgTbl.seat + 1
	local roomPlayer = self:GetPlayerByLocalseat(seatIdx)
	if roomPlayer == nil then return end
...
-- 准备晚了，座位被占了回调
function PlaySceneYS:onRcvReadyLate()
...
-- 玩家押分回调
function PlaySceneYS:onRcvPledge(msgTbl)
...
-- 提示押分
function PlaySceneYS:onPromptPledge(msgTbl)
	self.waitType = nil
	self.bgWaitingTip:setVisible(false)
	self:hideBuyPledge()
	self.autoWaitTime = msgTbl.time
...
function PlaySceneYS:onPromptBuyPledge(msgTbl)
...
function PlaySceneYS:onRcvBuyPledge(msgTbl)
...
--提示可以抢庄压倍率
function PlaySceneYS:onPromptRate(msgTbl)
	--self.rateBtnNode:setVisible(true)
	self.autoWaitTime = msgTbl.time
	self:hideBuyPledge()
...
--玩家抢庄压倍率回调
function PlaySceneYS:onRcvRate(msgTbl)
	if msgTbl.code == 1 then
...
-- 提示开始
function PlaySceneYS:onRcvPromptStart(msgTbl)
	self.roomState = ROOM_STATE.READY
...
-- 开始游戏回调
function PlaySceneYS:onRcvStart(msgTbl)
	if msgTbl.code == 0 then
		...
		if self.isWait then
			...
			if not self:isTwelve() then
				self.readyBtn:runAction(cc.MoveTo:create(0.5,cc.p(0,-115)))
			end	
		end
		if (not self.isWait) then
			self.inviteFriendBtn:setVisible(false)
...
-- 发牌
function PlaySceneYS:onRcvDealCard(msgTbl)
	if self:HasTrusteeship() then
		self.Btn_Trusteeship:setVisible(true)
	else
		self.Btn_Trusteeship:setVisible(false)
	...
	for j = 1,cardNum do
		for k,v in pairs(self.roomPlayers) do
			if not v.isWait then
				...
				-- if(self.tableSetting.game_type == gt.GameType.GAME_BANKER) then
				-- 	if not self.isWait and v.seatIdx == self.playerSeatIdx then
				-- 		local playerMe = self.roomPlayers[self.playerSeatIdx]
				-- 		if playerMe ~= nil then
				-- 			cardView.imgCards[j]:loadTexture(string.format("image/card2/%02x.png", playerMe.cards_in_hand[j]), ccui.TextureResType.plistType)
				-- 		end
				-- 	else
...
--发剩余的牌（抢庄后）
function PlaySceneYS:onRcvDealLeftCard(msgTbl)
...
--提示亮牌
function PlaySceneYS:onPromptShowCard(msgTbl)
	self.autoWaitTime = msgTbl.time
	if self.decisionBtnNode then
		self.decisionBtnNode:setVisible(false)
	end
...
--点击亮牌回调
function PlaySceneYS:onRcvShowCard(msgTbl)
...
--换庄回调
function PlaySceneYS:onRecvDealerSeat(msgTbl)
	--庄家座位号
	...
	if self.roomState > ROOM_STATE.READY and (self.tableSetting.game_type == gt.GameType.GAME_BANKER or self.tableSetting.game_type == gt.GameType.GAME_FREE_BANKER) then
		...
		if (num > 1) then
			...
			local func = function()
				...
				if roomPlayerBanker == nil then return end
				local playerInfoNode = gt.seekNodeByName(self.rootNode,"Node_playerInfo_" .. roomPlayerBanker.displaySeatIdx)
...
--玩家在线标识
function PlaySceneYS:onRcvOffLineState(msgTbl)
	local seatIdx = msgTbl.seat + 1
	local roomPlayer = self:GetPlayerByLocalseat(seatIdx)
	if roomPlayer == nil then return end
	local playerInfoNode = self.playerNodes[roomPlayer.displaySeatIdx]
...
--当前局数/最大局数量
function PlaySceneYS:onRcvRoundState(msgTbl)
	-- 牌局状态,剩余牌
...
--显示算牌中
function PlaySceneYS:ShowShowingCards(seat)
	seat = seat + 1
...
--语音，表情，文字消息
function PlaySceneYS:onRcvChatMsg(msgTbl)
	local weizhi = 0
	local seatIdx = msgTbl.seat + 1
...
function PlaySceneYS:showGunAnimation()
	if cc.UserDefault:getInstance():getStringForKey("showProp") ~= "false" then
...
--单局结算
function PlaySceneYS:onRcvRoundReport(msgTbl)
	gt.log("游戏结束")
...
-- 显示金币飞行动画
function PlaySceneYS:showCoinFlyTo(count,delay,fromPos,toPos,fromDisplayIdx,toDisplayIdx)
	...
	local callFun2 = function()
		local bankerPlayer = self:GetPlayerByLocalseat(self.bankerSeatIdx)
...
function PlaySceneYS:playAddScoreAni(playerIdx,addScore)
...
function PlaySceneYS:ClearAllAddScore()
	for i,v in ipairs(self.playerAddSocre) do
		v:setVisible(false)
	end
end

function PlaySceneYS:copyTab(st)
	local tab = {}
...
--总结算
function PlaySceneYS:onRcvFinalReport(msgTbl)
	gt.removeLoadingTips()
	--发送关闭解散房间事件
	...
	if self.isMatch > 0 then
		...
		gt.addBtnPressedListener(self.Btn_ViewFinalReport,function(sender)
			...
			self:addChild(finalReport,PlaySceneYS.ZOrder.REPORT)
		end)
	else
		self.lastRound = true
...
-- 显示胜负动画
function PlaySceneYS:showResultEffect(win)
	local sk
	if win then
...
-- 房间创建者解散房间
function PlaySceneYS:onRcvDismissRoom(msgTbl)
	gt.removeLoadingTips()
	if msgTbl.code == 1 then
		-- 房主直接解散
...
function PlaySceneYS:onRcvSponorVote(msgTbl)
	...
	if not applyDimissRoom then
		...
		self:addChild(applyDimissRoom,PlaySceneYS.ZOrder.DISMISS_ROOM,1000099)
	end
end

function PlaySceneYS:onRcvVote(msgTbl)
...
function PlaySceneYS:onRecvDouble(msg)
	if msg.code == 0 then
...
function PlaySceneYS:onRcvSyncScore(msgTbl)
	local player = msgTbl.player
	for i,v in pairs(self.roomPlayers) do
...
--更新当前时间
function PlaySceneYS:updateCurrentTime()
...
--更新电量和网络状态
function PlaySceneYS:updateBatteryAndNetwork()
...
--房间添加玩家
function PlaySceneYS:roomAddPlayer(roomPlayer)
	-- 播放声音
	self.playCDAudioID = gt.soundEngine:playEffect("common/ys/sit",false)

	if not roomPlayer then return end
	if not roomPlayer.displaySeatIdx then return end
...
-- 房间信息按钮回调
function PlaySceneYS:infoBtnClickEvt()
    local roomInfo = require("app/views/RoomInfo"):create(self.tableSetting,nil,self:isTwelve())
	self:addChild(roomInfo,PlaySceneYS.ZOrder.SETTING)
end

--发送玩家准备请求消息
function PlaySceneYS:readyBtnClickEvt()
	self.readyBtn:setVisible(false)
	if self.isMatch > 0 and self.Btn_ViewFinalReport then
    	self.Btn_ViewFinalReport:setVisible(false)
    end
	--这里不能移动  要在准备成功回调里面处理  有可能准备不成功 这个时候准备按钮还是显示的
	--self.startBtn:stopAllActions()
	--self.startBtn:runAction(cc.MoveTo:create(0.5, cc.p(0, -110)))
	for i = 1,self.roomChairs do
...
-- 开始游戏
function PlaySceneYS:startBtnClickEvt()
	self.startBtn:setVisible(false)
	local msgToSend = {}
...
-- 点击亮牌
function PlaySceneYS:showCardBtnClickEvt()
	self.showCardBtn:setVisible(false)
...
-- 点击搓牌
function PlaySceneYS:rubCardBtnClickEvt()
	local playerMe = self:GetPlayerByLocalseat(self.playerSeatIdx)
	local cards = {}
	if self.tableSetting.game_type == gt.GameType.GAME_BANKER then
		--抢庄牛牛
	...
	self:addChild(rubCardsAni,PlaySceneYS.ZOrder.SETTING)
end

-- 点击翻牌
function PlaySceneYS:openCardBtnClickEvt()
	self.openCardBtn:setVisible(false)
	self.rubCardBtn:setVisible(false)
	local playerMe = self:GetPlayerByLocalseat(self.playerSeatIdx)
...
--玩家进入准备状态
function PlaySceneYS:playerGetReady(seatIdx)
	local roomPlayer = self:GetPlayerByLocalseat(seatIdx)
	if roomPlayer == nil then return end
	--玩家状态改成准备状态
	...
	if seatIdx == self.playerSeatIdx then
		...
		self.readyBtn:stopAllActions()
		self.readyBtn:setVisible(false)
		if not self.isWait and not self:isTwelve() then
			self.startBtn:stopAllActions()
			self.startBtn:runAction(cc.MoveTo:create(0.5,cc.p(0,-110)))
		end
		self:hideSigns()
		...
		if (self.curRound <= 1 and self.roomState <= ROOM_STATE.START) then
			if (self:isBanker()) then
				...
				--准备后自己不是庄家则不显示开始按钮，则提示等待房主开始游戏
				local roomPlayfoerBanker = self:GetPlayerByLocalseat(self.bankerSeatIdx)
				local create
				for i,v in pairs(self.roomPlayers) do
				...
				local tip
				if create then
					tip = string.format("等待%s开始游戏...",create.nickname)
				else
					tip = string.format("等待房主开始游戏...")
				end
				self.lbWatingTip:setString(tip)
...
--隐藏所有玩家准备手势标识
function PlaySceneYS:hidePlayersReadySign()
	for i = 1,self.roomChairs do
...
--隐藏所有玩家闲家买码标识
function PlaySceneYS:hidePlayersBuyPledgeSign()
	if self:isTwelve() then
		return
	end
	for i = 1,self.roomChairs do
...
--隐藏所有玩家压分标识
function PlaySceneYS:hidePlayersPledgeSign()
	for i = 1,self.roomChairs do
		self.pledgeSigns[i]:setVisible(false)
	end
end
...
--隐藏所有玩家抢庄倍数标识
function PlaySceneYS:hidePlayersRateSign()
	for i = 1,self.roomChairs do
		self.bflRateSigns[i]:setVisible(false)
	end
end
...
--隐藏所有玩家庄家标识
function PlaySceneYS:hidePlayersBankerSign()
	for i = 1,self.roomChairs do
...
--隐藏所有玩家等待标识
function PlaySceneYS:hidePlayersWaitSign()
	for i = 1,self.roomChairs do
...
--隐藏所有玩家抢庄亮框图
function PlaySceneYS:hidePlayersRobZhuangFrame()
	for i = 1,self.roomChairs do
		local robZhuangSpr = self.playerRobZhuangFrames[i]
...
--隐藏牛类型并停止相应动作
function PlaySceneYS:hideNiuTypeActions()
	self.cardShowQueue = {}
	for i = 1,self.roomChairs do
		self.cardViews[i].imgNiuTypeBg:setVisible(false)
...
--隐藏牌型并停止相应动作
function PlaySceneYS:hideCardViewsActions()
	for i = 1,self.roomChairs do
		self.cardViews[i].node:setVisible(false)
		self.cardViews[i].imgNiuType:stopAllActions()
...
--隐藏牌型
function PlaySceneYS:hideCardViews()
	for i = 1,self.roomChairs do
		self.cardViews[i].node:setVisible(false)
	end
end
...
--清除牛牛特效
function PlaySceneYS:clearNiuEffect()
	for i = 1,self.roomChairs do
...
-- 每局准备后隐藏相应标识
function PlaySceneYS:hideSigns()
	-- 隐藏压分标识
	self:hidePlayersPledgeSign()
	-- 隐藏压倍率标识
	self:hidePlayersRateSign()
...
--重置牌型位置
function PlaySceneYS:resetCardsPos()
	for i = 1,self.roomChairs do
		for j = 1,5 do
...
--重置手牌明暗标识
function PlaySceneYS:resetCardSigns()
	for key,var in pairs(self.roomPlayers) do
...
--房间是否已坐满员
function PlaySceneYS:isSeatFull()
	local bFull = false
	local playerNum = gt.getTableSize(self.roomPlayers)
    if (self.roomChairs == gt.GameChairs.SIX and playerNum == gt.GameChairs.SIX) 
    	or (self.roomChairs == gt.GameChairs.TEN and playerNum == gt.GameChairs.TEN)
    	or (self.roomChairs == gt.GameChairs.EIGHT and playerNum == gt.GameChairs.EIGHT)
    	or (self.roomChairs == gt.GameChairs.TWELVE and playerNum == gt.GameChairs.TWELVE)
    	 then
        bFull = true
    end
...
function PlaySceneYS:setShowCardsShow(displaySeatIdx,cardsigns)
	local cardView = self.cardViews[displaySeatIdx]
...
function PlaySceneYS:setShowCardsPos(niuType,displaySeatIdx)
	local cardView = self.cardViews[displaySeatIdx]
	local playerMe = self:GetPlayerByLocalseat(self.playerSeatIdx)
	local bFull = self:isSeatFull()
	--先把自己的牌间距重新设置
...
-- 显示玩家具体信息面板
function PlaySceneYS:showPlayerInfo(sender)
	...
	self:addChild(playerInfoTips,PlaySceneYS.ZOrder.PLAYER_INFO_TIPS)
end


--出牌倒计时
function PlaySceneYS:playTimeCDStart(timeDuration,appear)
...
--更新出牌倒计时
function PlaySceneYS:playTimeCDUpdate(delta)
...
--将消息转换为花色和牌型
function PlaySceneYS:changePk(poker)
	local value
	if poker < 44 then
...
function PlaySceneYS:rubCardOverEvt()
	self:openCardBtnClickEvt()
end

function PlaySceneYS:playSceneResetEvt()
...
function PlaySceneYS:reInitScene()

	self.waitType = nil
	gt.dispatchEvent(gt.EventType.CLOSE_APPLY_DISMISSROOM)

	for i,v in ipairs(self.playerRobZhuangFrames) do
		v:stopAllActions()
	end
	if self.decisionBtnNode then
		self.decisionBtnNode:setVisible(false)
...
function PlaySceneYS:playSceneRestartEvt()
	...
	self.cardShowDelay = 0
	self.roomState = ROOM_STATE.INIT
	--self.readyBtn:setPositionX(self.RoomCreaterId==gt.playerData.uid and 790 or 640)
	--self.startBtn:setPositionX(self.RoomCreaterId==gt.playerData.uid and 490 or 640)
	--self.startBtn:setEnabled(false)
	--self.readyBtn:setVisible(true)
	--self.startBtn:setVisible(self.RoomCreaterId==gt.playerData.uid)

	gt.socketClient:clearMessage()
	self:reLogin()
end

function PlaySceneYS:backMainSceneEvt(eventType)
	extension.callBackHandler = {}
	-- 消息回调
...
function PlaySceneYS:IsSameIp(msgTbl)
...
-- 创建房间时是否开启搓牌
function PlaySceneYS:isOpenRubCard()
...
--显示房间玩法标题
function PlaySceneYS:showPlayTitle()
	--if(self.roomChairs == gt.GameChairs.SIX) then
	local str = ""
	...
	elseif (self.tableSetting.game_type == gt.GameType.GAME_BULL) then
		str = strGameType .. "  底分" .. self.tableSetting.base_score
	elseif (self.tableSetting.game_type == gt.GameType.GAME_GOLD) then
		...
		str = strGameType .. "  " .. strBetNum
	elseif (self.tableSetting.game_type == gt.GameType.GAME_BANKER) then
		str = strGameType .. "  倍数" .. (self.tableSetting.loot_dealer)
...
--显示庄家标识
function PlaySceneYS:showBankerSign()
	self:CloseTuiflag()
	local roomPlayerBanker = self:GetPlayerByLocalseat(self.bankerSeatIdx)
	if roomPlayerBanker == nil then return end
	local playerInfoNode = gt.seekNodeByName(self.rootNode,"Node_playerInfo_" .. roomPlayerBanker.displaySeatIdx)
...
--显示押分按钮
function PlaySceneYS:showScoreBtn(showDouble)

	local _max = 2
	gt.seekNodeByName(self.decisionBtnNode,"btnScore1"):setPositionX(-90.00)
	gt.seekNodeByName(self.decisionBtnNode,"btnScore2"):setPositionX(90.00)
	self.btnScoreTui:setPositionX(270.00)
	self.btnDoubleScore:setPositionX(270.00)

	local step_ = 80
	if self:isTwelve() then step_ = 60 end
	if self:isScoreSpecial11() then
	 	_max = 4 
		gt.seekNodeByName(self.decisionBtnNode,"btnScore1"):setPositionX(-3 * step_)
		gt.seekNodeByName(self.decisionBtnNode,"btnScore2"):setPositionX(-step_)
		gt.seekNodeByName(self.decisionBtnNode,"btnScore3"):setPositionX(step_)
		gt.seekNodeByName(self.decisionBtnNode,"btnScore4"):setPositionX(3 * step_)
	...
	if (self.scoreTui > 0) then
		if showDouble == 1 then
			if self:isScoreSpecial11() then
				...
				gt.seekNodeByName(self.decisionBtnNode,"btnScore1"):setPositionX(-5 * step_)
				gt.seekNodeByName(self.decisionBtnNode,"btnScore2"):setPositionX(-3 * step_)
				gt.seekNodeByName(self.decisionBtnNode,"btnScore3"):setPositionX(- step_)
				gt.seekNodeByName(self.decisionBtnNode,"btnScore4"):setPositionX(step_)
				self.btnScoreTui:setPositionX(3* step_)
			else
				-- self.btnDoubleScore:setPositionX(450.00)
				gt.seekNodeByName(self.decisionBtnNode,"btnScore1"):setPositionX(-270.00)
				gt.seekNodeByName(self.decisionBtnNode,"btnScore2"):setPositionX(-90.00)
...
--根据房间设置显示压分按钮
function PlaySceneYS:setShowPledgeScoreBtn()
	local _max = 2
	if self:isScoreSpecial11() then
		_max = 4
	end
...
--根据房间设置显示抢庄倍数按钮
function PlaySceneYS:setShowRateBtn()
	local nodeRateList = {}
	for i = 1,5 do
...
-- 开始录音
function PlaySceneYS:startAudio()
...
function PlaySceneYS:getVoiceUrl(respJson)
...
--停止录音
function PlaySceneYS:stopAudio(cancel)
...
--是否有托管功能
function PlaySceneYS:HasTrusteeship()
...
function PlaySceneYS:GetPlayerIndexByDisplayseat(displayseat_)
	local seatIndex_ = 0
	for i,v in pairs(self.roomPlayers) do
...
function PlaySceneYS:GetPlayerMe()
	local playerMe 
	for i,v in pairs(self.roomPlayers) do
...
function PlaySceneYS:GetPlayerByLocalseat(localseat)
	for i,v in pairs(self.roomPlayers) do
...
function PlaySceneYS:ShowTuiflag(localSeat,score)
	if score<=0 then
		return
	end
	local playerNode=self.playerNodes[self:GetPlayerByLocalseat(localSeat).displaySeatIdx]
	playerNode.TuiFlag:setVisible(true)
	playerNode.TuiScore:setString(tostring(score))
end

--关闭所有人的可推注xxx标识
function PlaySceneYS:CloseTuiflag()
	for i,v in ipairs(self.playerNodes) do
		v.TuiFlag:setVisible(false)
	end
end

function PlaySceneYS:onRcvPush(msgTbl)
	for i,v in ipairs(msgTbl.player) do
