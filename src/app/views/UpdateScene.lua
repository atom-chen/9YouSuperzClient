...
local function removeFile( path )
    local state= io.writefile(path, "")
    if not state then
        gt.UpLoadMsg(string.format("writefile %s error",path))
    end
...
local function renameFile(path, newPath)
    removeFile(newPath)
    local state,err=os.rename(path, newPath)
    if not state then
        gt.UpLoadMsg(tostring(err))
    end
...
function UpdateScene:ctor()
	...
	local skeletonNode = sp.SkeletonAnimation:create("image/login/logo.json", "image/login/logo.atlas")
	skeletonNode:setAnimation(0, "animation", true)
	gt.seekNodeByName(csbNode,"Node_Logo"):addChild(skeletonNode)
    -- 是否更新成功,更新失败会有tip提示
...
function UpdateScene:endProcess( endInfo )
    ...
    if self.updateSuccess == false then
        require("app/views/NoticeTipsForUpdate"):create("更新失败",
            "更新失败,请检查您的网络连接\n"..endInfo,
