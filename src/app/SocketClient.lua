...
function SocketClient:ctor(socketType)
	-- 加载消息打包库
...
function SocketClient:connect(serverIp, serverPort, isBlock)
	...
	-- 和服务器建立tcp链接
	tcpConnection:settimeout(isBlock and 10 or 0)
...
function SocketClient:registerMsgListener(msgId, msgTarget, msgFunc)
	...
	-- gt.messageManager:registerPreMsgListener(msgId)
end
...
function SocketClient:unregisterMsgListener(msgId)
	...
	-- gt.messageManager:unregisterPreMsgListener(msgId)
end
...
function SocketClient:dispatchMessage(msgTbl)
	-- gt.messageManager:dispatchPreMessage(msgTbl)
	local rcvMsgListener = self.rcvMsgListeners[msgTbl.cmd]
	if msgTbl.cmd ~= gt.HEARTBEAT then
		gt.log("--------dispatchMsg", msgTbl.cmd)
		if gt.localVersion then
			-- gt.dump(msgTbl)
		end
	end
