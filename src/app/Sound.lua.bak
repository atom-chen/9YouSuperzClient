...
function Sound:playEffect(fileName, isLoop)
	...
	local filePath = string.format("sound/%s.mp3", fileName)
...
function Sound:playMusic(fileName, isLoop)
	...
	isLoop = isLoop or false
	self:stopMusic()
	self.musicAudioID = ccexp.AudioEngine:play2d(string.format("sound/%s.mp3", fileName), isLoop, self.musicVolume * 0.01)
end
